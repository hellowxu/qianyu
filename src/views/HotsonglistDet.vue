      ...
      <div class="j-frameStyle">
        ...
        <div id="j-content">
          <div
            ...
            v-for="item in list"
            :key="item.id"
            @click="enterhotSongListSec(item.id)"
          >
  ...
  methods: {
    getHotSong() {
      axios.get("top/playlist?limit=10&order=hot").then(result => {
        this.list = result.data.playlists;
      });
    },
    hide() {
      this.showit = false;
    ...
    enterhotSongListSec(id) {
      // console.log(id);
      bus.$emit("enterhotsongsec", id);
